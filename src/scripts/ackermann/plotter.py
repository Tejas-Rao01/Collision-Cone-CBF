...
if no == -1:
    ...
    log_files = [file for file in files if re.match(r'log(\d+)\.txt', file)]

    if log_files:
        no = max(int(re.search(r'log(\d+)\.txt', file).group(1)) for file in log_files)
        print(no)
    else:
